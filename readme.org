...
A game written in Fennel + Love2D for the  [[https://itch.io/jam/autumn-lisp-game-jam-2025][Autumn Lisp Game Jam 2025]]. Based on the [[https://codeberg.org/alexjgriffith/min-love2d-fennel]] project.
...
In the lead up to the semi-annual [[https://itch.io/jam/autumn-lisp-game-jam-2018][Autumn Lisp Game Jam]] I thought I'd look into Phil Hagelberg's approach to last Aprils Jam, using [[https://love2d.org/][love2d]] in concert with [[https://fennel-lang.org/][fennel]]. Phil outlines his approach on his [[https://technomancy.us/187][blog]].

This repo contains the minimal viable setup to get started with Phil Hagelberg's game design process, plus some additional libraries.
...
If you want to just start coding up some fennel and love with no makefile or manual installation on linux check out [[https://codeberg.org/alexjgriffith/quick-love-fennel][quick-love-fennel]]

If you want to develop web first check out [[https://codeberg.org/alexjgriffith/fennel-lovejs-template][fennel-lovejs-template]]
...
#+BEGIN_SRC makefile
  ...
  URL=https://codeberg.org/alexjgriffith/min-love2d-fennel # Update
...
Once you install the latest version of [[https://git.sr.ht/~technomancy/fennel-mode][fennel-mode]], you can run
...
Fennel has its own [[https://git.sr.ht/~xerool/fennel-ls][language server]] and its now easier than ever to get it up and running with love2d!

First, download fennel-ls from its source, compile it and move it somewhere on your path.
...
#+BEGIN_SRC bash
  ...
  cd fennel-ls
  make
  mv fennel-ls somewhere-on-your-path
#+END_SRC

Double check =fennel-ls= is on your path and get the version. Our setup below should work with any version newer than 0.1.3.

#+BEGIN_SRC bash
  fennel-ls --version
#+END_SRC

If we try using =fennel-ls= now with our love2d project we are going to see a bunch of errors.

To get =fennel-ls= working well with love2d we need to add a =flsproject.fnl=
file at the root of your project (wherever the .git file is).

=fennel-ls= lets us include completions for libraries (such as love2d) with docsets. Fortunately for us [[https://git.sr.ht/~pinkstringmachine/][pinkstringmachine]] maintains a docset for love2d. Check out the [[https://wiki.fennel-lang.org/LanguageServer][fennel wiki]].

=fennel-ls= expects docsets to be located in a specific directory.
On linux it is =$HOME/.local/share/fennel-ls/docsets/=
Make sure this directory exists and then run the following, copied from the wiki.

#+BEGIN_SRC bash
    curl -o $HOME/.local/share/fennel-ls/docsets/love2d.lua https://p.hagelb.org/docsets/love2d.lua
#+END_SRC

Finally, we can add the following to =flsproject.fnl= to the root of our project.
#+BEGIN_SRC fennel
    ;; flsproject.fnl
...
Check out the [[https://git.sr.ht/~xerool/fennel-ls/tree/main/docs/manual.md][configuration]] section of the =fennel-ls= manual to see what other customization you can add to your =flsproject.fnl=.

* Phil's Modal Callbacks (PMC)

Phil Hagelberg's [[https://gitlab.com/technomancy/exo-encounter-667/][exo-encounter-667]] is structured using a modal callback system. Each game state has a mode and each mode has a series of specific callbacks.
